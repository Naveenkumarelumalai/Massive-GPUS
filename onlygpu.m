function [SER,poles,rmserr,fit,opts]=Approach2_onlygpu(f,s,poles,weight,opts);
%
...
if opts.relax==1   
 ...
 for i1=1:ds:Nc-1
         %---------%send to mex-----------
	    n1=min(Nc-1,ds+i1-1);
		%[R_gpu]=gpu_magma_multithread(Dk,weight(:,i1:n1),f1(:,i1:n1));
		[R_gpu]=gpu_magma(Dk,weight(:,i1:n1),f1(:,i1:n1));
		%[R_gpu]=gpu_cublas(Dk,weight(:,i1:n1),f1(:,i1:n1));
		R_Gpu=[R_Gpu;R_gpu];
end
  for n=Nc
